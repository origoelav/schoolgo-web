...
// 1. Replacements for student markers
const oldStudentBlock = `            const isCompleted = student.status === 'delivered' || student.status === 'collected';
...
// 2. Replacements for school markers
const oldSchoolBlock = `        el.innerHTML = \`
...
const newSchoolBlock = `        el.innerHTML = \`
...
// 3. Replacements for driver markers
const oldDriverBlock = `        el.innerHTML = \`
            <div style="pointer-events: auto; background: \${isOutOfRoute ? '#ef4444' : driverColor}; color: white; padding: 4px 10px; border-radius: 20px; font-size: 10px; font-weight: 900; white-space: nowrap; margin-bottom: 5px; transform: translateY(-30px); border: 2px solid white; box-shadow: 0 4px 10px rgba(0,0,0,0.2); display: flex; align-items: center; gap: 6px;">
                <span style="width: 6px; height: 6px; background: \${isOnline ? '#22c55e' : '#94a3b8'}; border-radius: 50%; display: inline-block; \dots
            </div>
        ...
        \`;`;
// We will replace using the precise original content string to avoid any differences.
